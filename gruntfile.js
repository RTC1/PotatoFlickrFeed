module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		options: {
			...
			css: {
				...
				min: '<%= options.publish %>/style.css'		// Name of the minified CSS file
			},
			...
			js: {
				...
				files: [
                    ...
                    'assets/js/filters/ordinalDate.js',
                    'assets/js/services/flickrService.js',
				...
				min: '<%= options.publish %>/script.js'		// Name of the minified JavaScript file
			},
			...
			watch: {
				files: [
                    '<%= options.js.files %>',
                    '<%= options.css.files %>',
                    '<%= options.less.base %>/*.less',
                    '<%= options.sass.base %>/*.sass',
                    '<%= options.sass.base %>/*.scss',
                    '<%= options.stylus.base %>/*.styl',
				    '!<%= options.js.min %>',
                    '!<%= options.less.compiled %>',
                    '!<%= options.sass.compiled %>',
                    '!<%= options.stylus.compiled %>'
	...
	// Register tasks
	grunt.registerTask('default', ['clean:all', 'less',  'concat:css', 'concat:js', 'cssmin', 'uglify', 'clean:concat', 'copy']); // Default task
}